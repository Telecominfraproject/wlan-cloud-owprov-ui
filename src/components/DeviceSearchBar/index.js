...
const DeviceSearchBar = ({ toggleEditModal }) => {
  ...
  const search = (value) => {
    if (socket.readyState === WebSocket.OPEN) {
      ...
      setSocket(new WebSocket(`${endpoints.owprov.replace('https', 'wss')}/api/v1/ws`));
    } else {
      setWaitingSearch(value);
    }
  };
  ...
  useEffect(() => {
    if (socket === null && endpoints?.owprov) {
      setSocket(new WebSocket(`${endpoints.owprov.replace('https', 'wss')}/api/v1/ws`));
    }
  }, []);

  return (
    <SearchBar t={t} search={search} results={results} history={history} action={toggleEditModal} />
  );
};
